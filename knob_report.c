...
static void work_left_button1_fn(struct work_struct *work)
{
	...
	input_report_key(knob_input_dev, BTN_0, 1);
...
{
	knob_input_dev->key[BIT_WORD(BTN_0)] = BIT_MASK(BTN_0);
	knob_input_dev->keybit[BIT_WORD(BTN_0)] = BIT_MASK(BTN_0);
	input_report_key(knob_input_dev, BTN_0, 0);
	input_sync(knob_input_dev);
...
static void work_right_button1_fn(struct work_struct *work)
{
	...
	input_report_key(knob_input_dev, BTN_1, 1);
	input_sync(knob_input_dev);
...
{
	knob_input_dev->key[BIT_WORD(BTN_1)] = BIT_MASK(BTN_1);
	knob_input_dev->keybit[BIT_WORD(BTN_1)] = BIT_MASK(BTN_1);
	input_report_key(knob_input_dev, BTN_1, 0);
	input_sync(knob_input_dev);
...
irqreturn_t irq_left_button1(int irq, void *dev_id)
{
	...
	if(val){
		work_left_button1.data.counter = val;
